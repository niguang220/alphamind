...
# 或推送到 Docker Hub
docker tag alphamind:latest username/alphamind:latest
docker push username/alphamind:latest
```
...
```bash
# 生产镜像
docker build -t alphamind:latest --target production .

# 开发镜像
docker build -t alphamind:dev --target development .
```
...
# 5. 在生产环境拉取运行
docker pull your-registry.com/alphamind:latest
./run-image.sh run --env-file .env.prod --detach
```
...
| 服务 | 地址 | 说明 |
|------|------|------|
| AlphaMind API | http://localhost:8000 | 主应用 API |

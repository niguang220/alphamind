  ...
  return (
    <aside className="flex h-full min-h-0 w-full flex-col border-l border-rule bg-desk">
      <div className="flex items-center justify-between border-b border-rule px-6 py-3">
        ...
        <div className="text-meta text-ink-faint">{turn ? 'this answer' : 'nothing yet'}</div>
      </div>
    ...
    <div className="space-y-7">
      ...
      <section>
        ...
        <div className="flex items-baseline gap-3">
          <span className="font-display text-display leading-none" style={{ color: primary.color }}>
            {primary.label}
          </span>
          ...
          <span className="tabular text-ui text-ink-faint">
            {guardrail ? 'rule' : turn.routing_score != null ? turn.routing_score.toFixed(2) : '—'}
          </span>
        </div>
        <p className="mt-1.5 text-ui text-ink-mute">{primary.blurb}</p>
        {supporting.length > 0 && (
      ...
      {scores.length > 0 && (
        <section>
            ...
            {scores.map(([name, val], i) => {
              ...
              return (
                <div key={name} className="flex items-center gap-3">
                  <span
                    className={`w-20 shrink-0 font-mono text-meta tracking-[0.06em] uppercase ${
                  ...
                  <span
                    className="tabular w-10 shrink-0 text-right text-meta"
          ...
          <p className="mt-2 text-meta leading-[1.5] text-ink-faint">
      ...
      {guardrail && (
        <section>
          ...
          <p className="border-l-2 border-brass/60 py-1 pl-3 text-ui leading-[1.6] text-brass/90">
                ...
                <div key={k}>
                  ...
                  <div className="mt-1 flex flex-wrap gap-1.5">
                    {v.map((x) => (
                      <span
                        key={x}
                        className="tabular border border-rule bg-raised px-2 py-[3px] text-meta text-ink-soft"
                      >
                        {x}
                      </span>
                    ))}
...
function Row({ k, children }) {
  return (
    <div className="flex items-baseline justify-between gap-4">
      ...
      <dd className="min-w-0 text-right text-ui text-ink-soft">{children}</dd>
    </div>
  )
}
...
function IdleState() {
  return (
    <p className="text-ui leading-[1.7] text-ink-mute">
  ...
  return (
    <section>
      <div className="flex items-baseline justify-between">
        ...
        <div className="tabular text-meta text-ink-faint">{ranAt}</div>
      </div>

      <div className="mt-3 space-y-2">
        {headline.map((m) => (
          <div key={m.key} className="flex items-baseline justify-between gap-3">
            <span className="text-ui text-ink-mute">{m.key}</span>
            <span className="tabular text-ui text-ink-soft">
              {m.pass}
      ...
      <div className="mt-4 space-y-1.5">
        {quality.map((m) => (
          <div key={m.key} className="flex items-center gap-3">
            <span className="label w-24 shrink-0">{m.key}</span>
            <div className="flex-1">
            ...
            <span className="tabular w-9 shrink-0 text-right text-meta text-ink-mute">
              {m.value.toFixed(2)}
            </span>
          </div>
        ))}
      ...
      <p className="mt-3 text-meta leading-[1.55] text-ink-faint">
